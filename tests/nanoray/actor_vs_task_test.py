# examples/actor_vs_task_benchmark.py
import argparse
import time
...
# ---------- Remote function (stateless) ----------
@remote()
def stateless_step(x, n: int, dtype_name: str, iters: int = 1):
    import torch
    dtype = getattr(torch, dtype_name)
    # Heavy state re-created every call (stateless)
...
class MatmulActor:
    def __init__(self, n: int, dtype_name: str, iters: int = 1):
        ...
        self.iters = iters
        # Heavy state built once and reused
...
def time_stateless(xs: List[torch.Tensor], n: int, dtype_name: str, iters: int):
    ...
    for x in xs:
        ...
        r = stateless_step.remote(x, n, dtype_name, iters)  # submit (execution starts)
        v = get(r)                                           # wait + collect
...
def time_actor(actor_ref, xs: List[torch.Tensor]):
    ...
    for x in xs:
        t0 = time.perf_counter()
        r = actor_ref.run.remote(x)  # submit (execution starts)
        v = get(r)                   # wait + collect
        t1 = time.perf_counter()
        times.append(t1 - t0)
        vals.append(v)
...
def main():
    ...
    # For REMOTE mode, override default addresses/tokens as needed
    ...
    try:
        ...
        # Actor creation (not timed in per-call numbers)
        ...
        # Owner preview (helps verify multi-node spread/placement)
